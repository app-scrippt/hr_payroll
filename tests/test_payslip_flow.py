...
from odoo.addons.hr_payroll_app_script.tests.common import TestPayslipBase
...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        context = {
            ...
            "active_ids": [self.ref("hr_payroll_app_script.menu_department_tree")],
            "active_id": self.ref("hr_payroll_app_script.menu_department_tree")
        }
        ...
        data, data_format = self.env.ref('hr_payroll_app_script.action_report_payslip').render(richard_payslip.ids)

        # I print the payslip details report
        data, data_format = self.env.ref('hr_payrhr_payroll_app_scriptoll.payslip_details_report').render(richard_payslip.ids)

        # I print the contribution register report
        context = {'model': 'hr.contribution.register', 'active_ids': [self.ref('hr_payroll_app_script.hr_houserent_register')]}
